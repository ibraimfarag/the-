      ...
      <table class="table table-striped table-bordered" >
          <tbody>
                <tr>
                    ...
                    @else
                        ...
                        <td>{{ format_price(abs($shop->current_balance)) }}</td>
                    @endif
                </tr>
                <tr>
        ...
        @if ($shop->current_balance > 0)
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-9">
                    @if ($seller_withdraw_request->requested_amount > $shop->current_balance)
                        <input type="number" lang="en" min="0" step="0.01" name="amount" id="amount" value="{{ abs($shop->current_balance) }}" class="form-control" required>
                    @else
                        <input type="number" lang="en" min="0" step="0.01" name="amount" id="amount" value="{{ $seller_withdraw_request->requested_amount }}" class="form-control" required>
                    @endif
                </div>
            </div>
